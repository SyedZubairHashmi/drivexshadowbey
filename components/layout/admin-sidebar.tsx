  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {user ? (user.role === 'admin' ? user.name?.charAt(0) || 'A' : `${user.ownerName?.charAt(0) || ''}${user.companyName?.charAt(0) || ''}`) : 'A'}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          ...
          >
            {user ? (user.role === 'admin' ? user.name : user.ownerName) : 'Admin'}
          </span>
          <span 
            style={{  
              color: '#009975',
          ...
          >
            {user ? (user.role === 'admin' ? user.email : user.companyEmail) : 'admin@drivexdeals.com'}
          </span>
        </div>
        <svg 
          onClick={logout}
